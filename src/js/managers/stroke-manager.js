<!DOCTYPE html>
<html lang="zh-CN">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0, maximum-scale=1.0, user-scalable=no">
    <title>学中文 | Learn Chinese</title>
    
    <!-- Content Security Policy - 限制资源加载来源，防止 XSS 攻击 -->
    <meta http-equiv="Content-Security-Policy" content="default-src 'self'; script-src 'self' 'unsafe-inline' https://www.googletagmanager.com; style-src 'self' 'unsafe-inline'; img-src 'self' data: https:; connect-src 'self' https://www.googletagmanager.com https://cdn.jsdelivr.net; font-src 'self' data:; frame-src 'self';">
    
    <!-- 预加载关键资源 -->
    <link rel="preload" href="src/css/style.css" as="style">
    <link rel="preload" href="src/js/app.js" as="script">
    <link rel="preload" href="src/js/lib/hanzi-writer.min.js" as="script">
    
    <!-- Base 标签：帮助浏览器正确解析相对路径 -->
    <!-- 注意：在本地开发时可能需要注释掉此行 -->
    <!-- <base href="./"> -->
    
    <!-- PWA 配置 -->
    <link rel="manifest" href="src/data/manifest.json">
    <meta name="theme-color" content="#4CAF50">
    
    <!-- 核心语言字体 (中文和英文) -->
    <style>
        /* 使用AR PL UKai字体 */
        @font-face {
            font-family: 'AR PL UKai';
            font-style: normal;
            font-weight: 400;
            src: local('AR PL UKai'), local('ARPLUKaiCN');
            font-display: swap;
        }
    </style>
    
    <!-- CSS -->
    <link rel="stylesheet" href="src/css/style.css?v=3.16">
    
    <!-- 第三方库 -->
    <!-- HanziWriter库 本地引用 -->
    <script src="src/js/lib/hanzi-writer.min.js"></script>
    
    <!-- html2canvas库用于截屏分享（动态加载，仅在需要时加载） -->
    
    <!-- 图标 -->
    <link rel="icon" type="image/png" href="./icon/icon-1024.png">
    <link rel="icon" type="image/png" sizes="192x192" href="./icon/icon-1024.png">
    <link rel="apple-touch-icon" href="./icon/icon-1024.png">
    <link rel="apple-touch-icon" sizes="180x180" href="./icon/icon-1024.png">
    
    <!-- iOS PWA 支持 -->
    <meta name="apple-mobile-web-app-capable" content="yes">
    <meta name="apple-mobile-web-app-status-bar-style" content="black-translucent">
    <meta name="apple-mobile-web-app-title" content="NihaoAPP">
    
    <!-- Android PWA 支持 -->
    <meta name="mobile-web-app-capable" content="yes">
    
    <!-- 描述信息 -->
    <meta name="description" content="学习中文的PWA应用,支持多种东南亚语言">
    <meta name="keywords" content="学中文,中文学习,越南语,印尼语,东南亚语言,PWA">
</head>
<body>
    <!-- 语言切换器 -->
    <div class="language-switcher" id="language-switcher" style="display: none;">
        <span class="language-flag" id="current-language-flag">🇻🇳</span>
        <span class="language-code" id="current-language-code">VI</span>
    </div>

    <!-- 语言选择模态框（首次访问显示） -->
    <div id="language-modal" class="modal">
        <div class="modal-content">
            <div class="modal-header">
                <p class="subtitle">选择母语 Your First Language</p>
            </div>
            
            <div class="language-grid" id="language-grid">
                <!-- 语言选项将通过JS动态生成 -->
            </div>
            
            <div class="modal-footer">
                <button id="confirm-language" class="primary-btn confirm-large" disabled>
                    确认 / confirm
                </button>
            </div>
        </div>
    </div>

    <!-- 主应用容器 -->
    <div id="app" class="app" style="display: none;">
        <!-- 三大卡片容器：占满主要高度 -->
        <div class="cards-container">
            <!-- 第1个卡片 -->
            <div class="card-1 card">
                <button id="theme-toggle" class="theme-toggle" aria-label="切换主题">🌙</button>
                
                <div class="chinese-content">
                    <div class="pinyin" id="pinyin">hǎo hǎo xué xí</div>
                    <div class="chinese" id="chinese">好好学习</div>
                </div>
                
                <button class="favorite-btn" id="favorite-btn" aria-label="收藏">☆</button>
            </div>

            <!-- 第2个卡片 -->
            <div class="card-2 card">
                <div class="translation" id="card-2-translation">Học tập chăm chỉ</div>
                <div class="english-translation" id="card-2-english">Study hard</div>
                <div class="explanation" id="card-2-explanation"></div>
            </div>

            <!-- 第3个卡片 -->
            <div class="card-3 card">
                <!-- 功能按钮 -->
                <div class="func-buttons">
                    <button class="func-btn" id="stroke-btn" title="写字/笔顺" aria-label="汉字写字">✏️</button>
                    <button class="func-btn" id="favorites-btn" title="词汇本" aria-label="查看收藏">📚</button>
                    <button class="func-btn" id="speech-btn" title="发音" aria-label="播放发音"><svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
   <path d="M11 5L6 9H2v6h4l5 4V5z"/>
   <path d="M15.54 8.46a5 5 0 010 7.07"/>
   <path d="M19.07 4.93a10 10 0 010 14.14"/>
 </svg></button>
                    <button class="func-btn" id="search-btn" title="搜索" aria-label="搜索词条">🔍</button>
                    <button class="func-btn" id="share-btn" title="分享" aria-label="分享网站">💬</button>
                </div>

                <!-- 桌面端底部导航链接 -->
                <div class="desktop-bottom-links">
                    <span>About Us</span> | 
                    <span class="nav-link" data-href="https://www.shutterstock.com/zh/g/guangyaohlg?rid=331009" target="_blank" rel="noopener noreferrer" aria-label="Image Content - 打开新窗口">Image Content</span> | 
                    <span class="nav-link" data-href="https://preply.com/en/?pref=MjQ4MDIwMTc=&id=1765940292.816984&ep=w2" target="_blank" rel="noopener noreferrer" aria-label="Real Tutor - 打开新窗口">Real Tutor</span> | 
                    <span>Follow me on X</span>
                </div>
            </div>
        </div>        

        <!-- 固定底部栏：字数按钮 + 导航提示 -->
        <div class="bottom-fixed-bar">
            <div class="length-selector">
                <button class="len-btn" data-length="1" aria-label="一字词">一</button>
                <button class="len-btn" data-length="2" aria-label="二字词">二</button>
                <button class="len-btn" data-length="3" aria-label="三字词">三</button>
                <button class="len-btn" data-length="4" aria-label="四字词">四</button>
                <button class="len-btn" data-length="5" aria-label="五字词">五</button>
            </div>
        </div>
    </div>

    <!-- 搜索模态框 -->
    <div id="search-modal" class="modal" style="display: none;">
        <div class="modal-content">
            <div class="modal-header">
                <h2>🔍 搜索 Search</h2>
            </div>
            <div class="modal-body">
                <input 
                    type="text" 
                    id="search-modal-input" 
                    placeholder="中文 / Pin Yin / English" 
                    autocomplete="off"
                    autofocus
                />
                <p class="hint">Pin Yin (hao hao xue xi) English (study hard) 中文（好好学习）</p>
            </div>
            <div class="modal-actions">
                <button class="btn-primary" id="confirm-search">搜索</button>
            </div>
        </div>
    </div>

    <!-- 词汇卡片网格容器 -->
    <div id="wordGrid" class="word-grid" style="display: none;">
        <!-- 动态生成的词汇卡片将插入这里 -->
    </div>

    <!-- 加载指示器 -->
    <div id="loadingIndicator" class="loading" style="display: none;">
        <div class="spinner"></div>
        <p>正在加载...</p>
    </div>

    <!-- 无结果提示 -->
    <div id="noResults" class="no-results" style="display: none;">
        <div class="no-results-content">
            <h3>没有找到匹配的词条</h3>
            <p>请尝试其他搜索关键词</p>
        </div>
    </div>

    <!-- Toast 提示 -->
    <div id="toast" class="toast" role="status" aria-live="polite" aria-atomic="true" style="display:none;"></div>

    <!-- 多汉字提示模态框 -->
    <div id="stroke-warning-modal" class="modal" style="display: none;">
        <div class="modal-content">
            <div class="modal-header">
                <h2>提示</h2>
            </div>
            <div class="modal-body">
                <div id="stroke-warning-content" class="stroke-warning-content"></div>
            </div>
            <div class="modal-actions">
                <button id="close-stroke-warning" class="btn-primary">确定</button>
            </div>
        </div>
    </div>

    <!-- 笔顺动画模态框 -->
    <div id="stroke-order-modal" class="modal" style="display: none;">
        <div class="modal-content">
            <div class="modal-header">
                <h2 id="stroke-modal-title">汉字笔顺</h2>
            </div>
            <div class="modal-body">
                <div id="stroke-animation-container" class="stroke-animation-container"></div>
                <div class="stroke-controls">
                    <button id="play-stroke" class="btn-primary" title="播放笔顺动画">▶</button>
                    <button id="practice-stroke" class="btn-secondary" title="练习书写">👆</button>
                </div>
            </div>
            <div class="modal-actions">
                <button id="close-stroke-modal" class="btn-secondary">✕</button>
            </div>
        </div>
    </div>

    <!-- 显示计数器 -->
    <div id="displayCount" class="display-count" style="display: none;">
        <span id="count-number">0</span>
    </div>

    <!-- 意见反馈模态框 -->
    <div id="feedback-modal" class="modal" style="display: none;">
        <div class="modal-content feedback-modal-content">
            <div class="modal-header">
                <h2 id="feedback-title">📝 意见反馈</h2>
                <p class="subtitle" id="feedback-subtitle">您的反馈对我们非常重要</p>
            </div>
            
            <div class="modal-body">
                <!-- 问题1：最喜欢的功能 -->
                <div class="feedback-section">
                    <label class="feedback-label" id="feedback-favorite-label">您最喜欢的功能是什么？</label>
                    <div class="feedback-options">
                        <label class="feedback-option">
                            <input type="radio" name="favorite-feature" value="stroke-order">
                            <span id="feedback-opt-stroke">汉字笔顺</span>
                        </label>
                        <label class="feedback-option">
                            <input type="radio" name="favorite-feature" value="speech">
                            <span id="feedback-opt-speech">语音发音</span>
                        </label>
                        <label class="feedback-option">
                            <input type="radio" name="favorite-feature" value="search">
                            <span id="feedback-opt-search">搜索功能</span>
                        </label>
                        <label class="feedback-option">
                            <input type="radio" name="favorite-feature" value="favorites">
                            <span id="feedback-opt-favorites">收藏词汇</span>
                        </label>
                    </div>
                </div>
                
                <!-- 问题2：是否愿意付费 -->
                <div class="feedback-section">
                    <label class="feedback-label" id="feedback-willingness-label">您愿意为高级功能付费吗？</label>
                    <div class="feedback-options">
                        <label class="feedback-option">
                            <input type="radio" name="payment-willingness" value="yes">
                            <span id="feedback-opt-yes">愿意</span>
                        </label>
                        <label class="feedback-option">
                            <input type="radio" name="payment-willingness" value="maybe">
                            <span id="feedback-opt-maybe">可以考虑</span>
                        </label>
                        <label class="feedback-option">
                            <input type="radio" name="payment-willingness" value="no">
                            <span id="feedback-opt-no">不愿意</span>
                        </label>
                    </div>
                </div>
                
                <!-- 问题3：期待的新功能 -->
                <div class="feedback-section">
                    <label class="feedback-label" id="feedback-features-label">您希望我们添加什么新功能？</label>
                    <div class="feedback-options">
                        <label class="feedback-option">
                            <input type="checkbox" name="new-features" value="ai-tutor">
                            <span id="feedback-feat-ai">AI导师辅导</span>
                        </label>
                        <label class="feedback-option">
                            <input type="checkbox" name="new-features" value="live-class">
                            <span id="feedback-feat-live">真人直播课</span>
                        </label>
                        <label class="feedback-option">
                            <input type="checkbox" name="new-features" value="ocr-scan">
                            <span id="feedback-feat-ocr">拍照OCR识字</span>
                        </label>
                        <label class="feedback-option">
                            <input type="checkbox" name="new-features" value="more-vocab">
                            <span id="feedback-feat-vocab">更多词汇内容</span>
                        </label>
                    </div>
                </div>
                
                <!-- 问题4：其他建议 -->
                <div class="feedback-section">
                    <label class="feedback-label" id="feedback-suggestion-label">其他建议（可选）</label>
                    <textarea id="feedback-suggestion" class="feedback-textarea" placeholder="请输入您的建议..."></textarea>
                </div>
                
                <!-- 进度指示器 -->
                <div class="feedback-progress">
                    <span id="feedback-progress-text">已收集 0/10 条反馈</span>
                </div>
                
                <!-- 导出按钮 -->
                <div class="feedback-export-section">
                    <button id="export-feedback" class="btn-secondary feedback-export-btn">
                        📥 <span id="feedback-btn-export">导出反馈文件</span>
                    </button>
                    <p class="feedback-export-hint" id="feedback-export-hint">下载后可发送到 X 私信给我</p>
                </div>
            </div>
            
            <div class="modal-actions feedback-modal-actions">
                <button id="skip-feedback" class="btn-secondary">
                    <span id="feedback-btn-skip">跳过</span>
                </button>
                <button id="submit-feedback" class="btn-primary">
                    <span id="feedback-btn-submit">提交</span>
                </button>
            </div>
        </div>
    </div>


    <!-- Google Analytics 4 -->
    <script async src="https://www.googletagmanager.com/gtag/js?id=G-XXXXXXXXXX"></script>

    <!-- Google Analytics 4 统计代码和推荐码追踪 -->
    <script>
        // ========================================
        // 请将 G-XXXXXXXXXX 替换为你的 GA4 测量ID
        // 获取方式：https://analytics.google.com/
        // ========================================
        window.dataLayer = window.dataLayer || [];
        function gtag(){dataLayer.push(arguments);}
        gtag('js', new Date());
        gtag('config', 'G-XXXXXXXXXX');

        // ========================================
        // 推荐码追踪功能
        // 页面加载时解析 URL 中的 ref 参数并推送到 GA4
        // ========================================
        function initReferralTracking() {
            // 从 URL 中获取推荐码
            const urlParams = new URLSearchParams(window.location.search);
            const refCode = urlParams.get('ref');

            if (refCode) {
                console.log('检测到推荐码:', refCode);

                // 将推荐码保存到 localStorage
                localStorage.setItem('referredBy', refCode);

                // 如果是首次访问，发送到 GA4
                const firstVisit = localStorage.getItem('firstVisitTime');
                if (!firstVisit) {
                    localStorage.setItem('firstVisitTime', Date.now().toString());

                    // 发送到 GA4 作为新用户事件
                    if (typeof gtag !== 'undefined') {
                        gtag('event', 'referral_signup', {
                            'referral_code': refCode,
                            'referral_source': 'share_link'
                        });

                        // 同时设置用户属性，用于在 GA4 后台按推荐码筛选
                        gtag('set', 'user_properties', {
                            referred_by: refCode
                        });
                    }

                    console.log('新用户推荐记录已发送到 GA4，推荐码:', refCode);
                }
            }
        }

        // 页面加载时初始化推荐追踪
        initReferralTracking();
    </script>

    <!-- 脚本 -->
    <script src="src/js/language.js"></script>
    <script src="src/js/font-loader.js"></script>
    <script src="src/js/theme.js"></script>

    <script type="module" src="src/js/app.js"></script>
    <script src="src/js/sw-register.js"></script>

    <script>
        // 获取设备提示的语言文本
        function getDeviceLanguageText() {
            const userLanguage = localStorage.getItem('userLanguage') || 'default';
            return deviceLanguageTexts[userLanguage] || deviceLanguageTexts['default'];
        }
        
        // 设备检测逻辑（统一使用这个函数）
        function isDesktopDevice() {
            // 检查是否为桌面设备的多种方法
            const userAgent = navigator.userAgent.toLowerCase();
            const isMobile = /android|webos|iphone|ipad|ipod|blackberry|windows phone|mobile/.test(userAgent);
            const hasTouch = 'ontouchstart' in window || navigator.maxTouchPoints > 0;
            const screenWidth = window.innerWidth;
            
            // 如果不是移动设备，且不支持触摸，且屏幕宽度足够大，则判断为桌面设备
            return !isMobile && !hasTouch && screenWidth > 768;
        }
        
        // 显示设备提示模态框
        function showDeviceModal() {
            const deviceModal = document.getElementById('device-modal');
            if (deviceModal) {
                deviceModal.style.display = 'flex';
                
                // 根据用户语言更新模态框内容
                const texts = getDeviceLanguageText();
                
                const titleElement = document.getElementById('device-modal-title');
                const messageElement = document.getElementById('device-message-native');
                const dismissButtonElement = document.getElementById('dismiss-button-text');
                
                if (titleElement) titleElement.textContent = texts.title;
                if (messageElement) messageElement.textContent = texts.message;
                if (dismissButtonElement) dismissButtonElement.textContent = texts.dismiss;
            }
        }
        
        // 关闭设备提示模态框
        function dismissDeviceModal() {
            const deviceModal = document.getElementById('device-modal');
            if (deviceModal) {
                deviceModal.style.display = 'none';
            }
        }
        
        // 页面加载时检测设备类型
        document.addEventListener('DOMContentLoaded', function() {
            // 为关闭按钮添加事件监听器
            const dismissBtn = document.getElementById('dismiss-device-notice');
            if (dismissBtn) {
                dismissBtn.addEventListener('click', dismissDeviceModal);
            }
            
            // 检查是否已经有选择的语言
            const userLanguage = localStorage.getItem('userLanguage');
            
            if (userLanguage) {
                // 如果已经选择了语言，直接显示设备提示
                if (isDesktopDevice()) {
                    showDeviceModal();
                }
            } else {
                // 如果还没有选择语言，监听语言选择事件
                window.addEventListener('languageSelected', function() {
                    if (isDesktopDevice()) {
                        showDeviceModal();
                    }
                });
            }
        });
        
        document.addEventListener('DOMContentLoaded', function() {
            console.log('学中文应用初始化...');
            
            if (typeof languageManager !== 'undefined') {
                languageManager.init().catch(err => {
                    console.error('❌ 语言管理器初始化失败:', err);
                });
            }
        });
    </script>
</body>
</html>
